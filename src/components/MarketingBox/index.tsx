...
const MarketingBox = (props) => {
  ...
  const handleClaim = useCallback(async () => {
    if (!wrapper) return;
    if (isNaN(Number(accountData?.marketingclaimable)) || Number(accountData?.marketingclaimable) <= 0) {
    ...
    NotificationManager.success(`${accountData?.marketingclaimable} ${tokenInfos.KATA.symbol} claimed`, 'Claim Success');

  }, [accountData, wrapper])
  
  const getClaimText = useCallback(() => {
    if (!accountData || !accountData.marketingclaimable) return "Nothing to Claim";
    return "Claim";
  }, [accountData])
          ...
          accountData.marketingkataBalance? (
              <div className='mt-3 '>
                <div className="d-flex justify-content-between px-1 mb-1">
                  ...
                  <h3 className="font-weight-bold claim-color">{toFixed(accountData.marketingkataBalance,2)} $KATA</h3>
                </div>
                <div className="d-flex justify-content-between px-1 mb-1">
                    <h5 className="claim-info">Claimed: </h5>
                    <h3 className="font-weight-bold text-right claim-color"> {toFixed(accountData.marketingclaimed, 2)} $KATA</h3>
                </div>
                <div className="d-flex justify-content-between px-1 mb-1">
                    <h5>Now Claim: </h5>
                    <h3 className="font-weight-bold text-right claim-now-color"> {toFixed(accountData.marketingclaimable, 2)} $KATA</h3>
                </div>
