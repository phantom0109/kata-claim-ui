...
const AdvisorBox = (props) => {
  ...
  const handleClaim = useCallback(async () => {
    if (!wrapper) return;
    if (isNaN(Number(accountData?.advisorclaimable)) || Number(accountData?.advisorclaimable) <= 0) {
    ...
    const txHash = await wrapper.airdropclaim();
    ...
    NotificationManager.success(`${accountData?.advisorclaimable} ${tokenInfos.KATA.symbol} claimed`, 'Claim Success');

  }, [accountData, wrapper])
  
  const getClaimText = useCallback(() => {
    if (!accountData || !accountData.advisorclaimable) return "Nothing to Claim";
    return "Claim";
  }, [accountData])
    ...
    <>
      {!accountData ? (
        ...
        />
          ...
          accountData.advisorkataBalance? (
              <div className='mt-3 '>
                <div className="d-flex justify-content-between px-1 mb-1">
                  <h5 className="claim-info">Purchased:</h5>
                  <h3 className="font-weight-bold claim-color">{toFixed(accountData.advisorkataBalance,2)} $KATA</h3>
                </div>
                <div className="d-flex justify-content-between px-1 mb-1">
                    <h5 className="claim-info">Claimed: </h5>
                    <h3 className="font-weight-bold text-right claim-color"> {toFixed(accountData.advisorclaimed, 2)} $KATA</h3>
                </div>
                <div className="d-flex justify-content-between px-1 mb-1">
                    <h5>Now Claim: </h5>
                    <h3 className="font-weight-bold text-right claim-now-color"> {toFixed(accountData.advisorclaimable, 2)} $KATA</h3>
                </div>
