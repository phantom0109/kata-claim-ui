...
const AirdropBox = (props) => {
  ...
  const handleClaim = useCallback(async () => {
    if (!wrapper) return;
    if (isNaN(Number(accountData?.treasuryclaimable)) || Number(accountData?.treasuryclaimable) <= 0) {
    ...
    NotificationManager.success(`${accountData?.treasuryclaimable} ${tokenInfos.KATA.symbol} claimed`, 'Claim Success');

  }, [accountData, wrapper])
  
  const getClaimText = useCallback(() => {
    if (!accountData || !accountData.treasuryclaimable) return "Nothing to Claim";
    return "Claim";
  }, [accountData])
          ...
          accountData.treasurykataBalance? (
              <div className='mt-3 '>
                <div className="d-flex justify-content-between px-1 mb-1">
                  ...
                  <h3 className="font-weight-bold claim-color">{toFixed(accountData.treasurykataBalance,2)} $KATA</h3>
                </div>
                <div className="d-flex justify-content-between px-1 mb-1">
                    <h5 className="claim-info">Claimed: </h5>
                    <h3 className="font-weight-bold text-right claim-color"> {toFixed(accountData.treasuryclaimed, 2)} $KATA</h3>
                </div>
                <div className="d-flex justify-content-between px-1 mb-1">
                    <h5>Now Claim: </h5>
                    <h3 className="font-weight-bold text-right claim-now-color"> {toFixed(accountData.treasuryclaimable, 2)} $KATA</h3>
                </div>
